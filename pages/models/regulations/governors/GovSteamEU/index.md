...
is used instead, see
<a href="#sec-emergency" class="quarto-xref">Section 3.2.3</a>.

The boiler output, i.e. the amount of steam supplied from the boiler to
the turbine, is manipulated by the control valves [2].
...
because there is no other following turbine stage) [2].
...
primary control paths [2], island and grid connected mode. They output
...
for the proportional $$K_\mathrm{\omega\,cor}$$ [2] [4].
...
to obtain the error signal $$\Delta f$$ of the PID controller [2] [4].
...
regular speed/load adjustments [2]. In overspeed situations, the
